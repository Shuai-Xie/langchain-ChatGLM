...
def add_vs_name(vs_name, chatbot):
    if vs_name is None or vs_name.strip() == "":
        ...
        if not os.path.exists(
            os.path.join(KB_ROOT_PATH, vs_name, "vector_store")
        ):
...
def reinit_vector_store(vs_id, history):
    try:
        ...
        vs_path, loaded_files = local_doc_qa.init_knowledge_vector_store(
            os.path.join(KB_ROOT_PATH, vs_id, "content"), vs_path, sentence_size
        )
...
def delete_file(vs_id, files_to_delete, chatbot):
    ...
    status = local_doc_qa.delete_file_from_vector_store(
        vs_path=vs_path, filepath=docs_path
    )
    ...
    return (
        gr.update(
            choices=local_doc_qa.list_file_from_vector_store(vs_path), value=[]
        ),
        chatbot,
    )
...
with gr.Blocks(
    ...
    vs_path, file_status, model_status = (
        gr.State(
            os.path.join(KB_ROOT_PATH, get_vs_list()[0], "vector_store")
        ...
        with gr.Row():
            ...
            with gr.Column(scale=5):
                ...
                with vs_setting:
                    ...
                    vs_refresh.click(
                        fn=refresh_vs_list, inputs=[], outputs=select_vs_test
                    )
                    vs_add.click(
                        fn=add_vs_name,
                        inputs=[vs_name, chatbot],
